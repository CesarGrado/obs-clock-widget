...
function dateTimeFormatter(locales: Intl.LocalesArgument | undefined, options: Intl.DateTimeFormatOptions): Intl.DateTimeFormat {
  const key = `${String(locales)}|${JSON.stringify(options)}`;
  let formatter = dateTimeFormatters.get(key);
  if (!formatter) { formatter = new Intl.DateTimeFormat(locales, options); dateTimeFormatters.set(key, formatter); }
  return formatter;
}

function numberFormatter(locale: string | undefined, minimumIntegerDigits: number): Intl.NumberFormat {
  const key = `${String(locale)}|${minimumIntegerDigits}`;
  let formatter = numberFormatters.get(key);
  if (!formatter) {
    formatter = new Intl.NumberFormat(locale, { useGrouping: false, minimumIntegerDigits, maximumFractionDigits: 0 });
...
export function formatClock(date: Date, format: string, timezone: string, locale: string): string {
  ...
  const parts = dateTimeFormatter('en-US-u-ca-gregory-nu-latn', { timeZone, year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hourCycle: 'h23' }).formatToParts(date);
  ...
  const name = (month: 'long' | 'short' | undefined, weekday: 'long' | 'short' | undefined) => dateTimeFormatter(lang, { timeZone, calendar: 'gregory', month, weekday }).format(date);
  ...
  const values: Record<string, string> = {
    ...
    a: dateTimeFormatter(lang, { timeZone, calendar: 'gregory', hour: 'numeric', hour12: true }).formatToParts(date).find((part) => part.type === 'dayPeriod')?.value ?? (h24 < 12 ? 'AM' : 'PM'),
  };
...
export const formatLocalizedNumber = (value: number, locale?: string, minimumIntegerDigits = 1): string => numberFormatter(locale, minimumIntegerDigits).format(value);
