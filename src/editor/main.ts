...
function buildLine(parent: HTMLElement, n: number) {
  ...
  const presets = select(`line${n}-format-preset`, ['', 'HH:mm:ss', 'h:mm:ss a', 'h:mm a', 'dddd, MMMM D, YYYY', "HH:mm 'UTC'"]); presets.options[0]!.text = 'Custom'; labeled(section, 'Format preset', presets);
